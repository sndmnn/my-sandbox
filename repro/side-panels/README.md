...
As you can see this hook also returns the "panel stack" as a `JSX.Element` array that can be rendered in the parent component. This array is intended to be rendered in the parent component wrapped in a styled component called `SidePanelStack`, like so:
...
My problem is whenever `MyPanelContent`, however it may be called, receives props that change its state, the side panel doesn't re-render.
